#!/bin/bash
## This script will make a whole server export of the past 7 days. ##TOKEN AND OUTPUT DIRECTORY NEED TO BE FILLED OUT MANUALLY FIRST##
## 
...
## DATE="`date +%B-%d-%Y`"
##
EXPORT_DATE=`(date --date="last week UTC" '+%Y-%m-%d-00:00')` ## Will output the date of 7 days ago of your current time at 00:00, adjusted to the timezone (UTC in this case, change as you need). 
...
SERVER_ID='' ## No need to fill this part
TOKEN='' ## Important: keep token between ''
OUTPUT_DIRECTORY="" ## Make sure this path doesn't have a slash at the end. // No need to fill this part
CHAT_EXPORTER_DIRECTORY='' ## The directory of the chat exporting tool.  Keep a slash at the end for this directory. Keep the path between ''
